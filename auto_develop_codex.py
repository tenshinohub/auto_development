    ...
    def __init__(
        ...
        try:
            ...
            print(
                ...
                "local subject adjustments are disabled."
            )
...
def make_region_masks(
    ...
    # These provisional masks depend on fixed vertical coordinates rather
    # than true sky/water segmentation.  They can create a visible horizontal
    # colour boundary, so do not use them for development until a semantic
    # sky/water model is available.
    water = np.zeros_like(subject, dtype=bool)
    upper_bright = np.zeros_like(subject, dtype=bool)
...
def classify_scene(
    ...
    # Landscape
    if (
        # Focal length is commonly absent when exiftool is unavailable.
        # A low-warmth, low-edge frame with meaningful dynamic range is still
        # a reliable exterior/landscape signal (e.g. a blue sky and skyline).
        (
            shooting.wide_angle
            or (
        ...
        # dynamic_range is log10(p95 / p05), so 5.0 is effectively
        # unreachable for normally exposed sRGB analysis data.  The old
        # threshold prevented every practical landscape from using its
        # dedicated exposure profile.
        and stats.dynamic_range > 0.50
    ):
        return SceneResult(
            scene="landscape",
            confidence=0.70,
...
def calculate_exposure_target(
    ...
    targets = {
        # These targets are measured in the sRGB analysis image.  The old
        # 0.20--0.22 values deliberately selected a near-black midtone,
        # despite the RAW data often having ample highlight headroom.
        "portrait": 0.270,
        "night": 0.130,
        "sunset": 0.240,
        "landscape": 0.340,
...
def estimate_exposure_ev(
    ...
    # Bright skies, specular reflections and pale flowers can legitimately
    # fill the upper percentiles.  Use midtones to set exposure and reserve
    # highlight handling for the later roll-off curve.
    highlight_soft = 0.880
    highlight_hard = 0.980
...
def apply_tone(
    ...
    # Keep the contrast curve subtle.  The former value darkened every tone
    # below middle gray before shadow recovery, which made low-key scenes
    # muddy and strongly underexposed bright-sky scenes.
    s = (
        y
        + strength
        * 0.055
...
def apply_region_processing(
    ...
    # Do not call the entire frame "background" when no reliable subject was
    # detected.  That case is common for landscapes and close-up flowers.
    if np.any(background) and np.any(subject):
        bg = out[background]
    ...
    if np.any(water):
        # Scene profiles express water_saturation as a small adjustment
        # (for example 0.08 means +8%), not an absolute saturation value.
        # Applying 0.08 as a multiplier almost removed all colour from the
        # lower part of a sky/water mask.  Feather the mask as well, so a
        # heuristic spatial boundary cannot form a visible horizontal seam.
        sigma = max(4.0, min(out.shape[:2]) * 0.01)
        ...
        adjusted = (
            y[..., None]
            + (out - y[..., None])
            * (1.0 + params.water_saturation)
        )

        out = (
            out * (1.0 - weight[..., None])
            + adjusted * weight[..., None]
        )
...
def score_candidate(
    ...
    # A small bright region is not a reason to darken the whole photograph.
    # Highlights are compressed later by apply_tone().
    if stats.p95 > 0.88:
        score -= (
            stats.p95 - 0.88
        ) * 2.0
...
def automatic_parameter_search(
    ...
    # Candidate scoring has several competing terms.  In non-night scenes,
    # never let those terms choose an exposure whose midtone is materially
    # below the scene target; it causes unpredictable dark jumps between runs.
    source_median = calculate_stats(image).median
    if scene == "night":
        minimum_ev = -1.0
    else:
        minimum_ev = math.log2(
            max(target * 0.90, 1e-5)
            / max(source_median, 1e-5)
        )
    ...
    # The scene profile must influence the search.  Previously its exposure,
    # contrast and saturation entries were ignored, so every scene was
    # restricted to nearly neutral saturation (at most 1.03).
    base_contrast = float(profile.get("contrast", 1.0))
    contrasts = [
        clamp(base_contrast * factor, 0.94, 1.12)
        for factor in (0.97, 1.00, 1.03, 1.06)
    ]

    base_saturation = float(profile.get("saturation", 1.0))
    saturations = [
        clamp(base_saturation * factor, 0.94, 1.12)
        for factor in (0.98, 1.03, 1.07)
    ]
    ...
    for offset in offsets:
        ev = clamp(
            max(
                estimated_ev
                + float(profile.get("exposure", 0.0))
                + offset,
                minimum_ev,
            ),
            -1.0,
